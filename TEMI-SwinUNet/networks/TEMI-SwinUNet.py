...
class MSAA(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # 定义卷积层来调整 x1, x2, x4 的通道数
        self.conv1 = nn.Conv2d(96, out_channels, kernel_size=1)
        self.conv2 = nn.Conv2d(192, out_channels, kernel_size=1)

        # 原先的 FusionConv，可以接受通道数一致的输入
        self.fusion_conv = FusionConv(out_channels * 3, out_channels)

    def forward(self, x1, x2,x3,  last=False):
        # 调整通道数
        x1 = self.conv1(x1)
        x2 = self.conv2(x2)
        size = x3.size()[2:]  # 获取 x1 的高度和宽度
        ...
        x_fused = x_fused.reshape(x_fused.size(0), -1, x_fused.size(1))
        return x_fused
def gcd(a, b):
    while b:
        a, b = b, a % b
...
class SwinTransformerSys(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # 修改
        self.msaa = MSAA(in_channels=384, out_channels=384)
        self.mscb = MSCBLayer(in_channels=768, out_channels=384)
        #self.ag = AG(intermediate_channels=32)
        self.conv_transpose = nn.ConvTranspose2d(in_channels=384, out_channels=384, kernel_size=4, stride=2, padding=1)
        #
    ...
    def forward_features(self, x):
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if idx == 2:  # 在第三层之后插入
                ...
                x = self.msaa(x_downsample[0].permute(0, 3, 1, 2),
                              x_downsample[1].permute(0, 3, 1, 2),
                              x_downsample[2].permute(0, 3, 1, 2))

            x = layer(x)
        x = self.norm(x)  # B L C

        return x, x_downsample1
...
if __name__ == '__main__':
    block = SwinTransformerSys(features=3, out_features=3)
    input_tensor = torch.rand(24, 3, 224, 224)
